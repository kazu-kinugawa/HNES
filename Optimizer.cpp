...
void Optimizer::sgd(const Real& grad, const Real learningRate, Real& param){
  param -= learningRate * grad;
}
...
void Optimizer::adagrad(Real& grad, const Real learningRate, Real& gradHist, Real& param){
  gradHist += grad * grad;
  grad /= sqrt(gradHist);
  Optimizer::sgd(grad, learningRate, param);
}
...
void Optimizer::momentum(Real& grad, const Real learningRate, const Real m, Real& gradHist, Real& param){
  gradHist *= m;
  Optimizer::sgd(grad, -learningRate, gradHist);
  Optimizer::sgd(gradHist, 1.0, param);
}
