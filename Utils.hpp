...
namespace Utils {
  ...
  inline void split(const std::string& str, std::vector<std::string>& res, const char sep){
    //A sentence "str" is split by character "sep", into vector "res"
    bool tok = false;
    int beg = 0;
    ...
    for (int i = 0, len = str.length(); i < len; ++i) {
      if (!tok && str[i] != sep) {//tok == false && str[i] != sep
	beg = i;
	tok = true;
      }

      if (tok && (i == len-1 || str[i] == sep)) {//tok == true && (str[i] == sep || i == len - 1)
	tok = false;
  ...
  /*
  // cannot use this function at pigeon0
  ...
  */
  ...
  inline void save(std::ofstream& ofs, const Real& params){
      ofs.write((char*)&params, sizeof(Real));
  }
  ...
  inline void load(std::ifstream& ifs, Real& params){
    Real val = 0.0;
      ifs.read((char*)&val, sizeof(Real));
      params = val;
  }
